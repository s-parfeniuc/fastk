...
pub mod estab {
    ...
    pub fn fetch_update_stability(
        ...
        match a.fetch_update(Ordering::SeqCst, Ordering::Relaxed, |cur| {
            ...
            if (new_est < est && est - x <= old_est) || (s < 0 && new_est < est) {
...
pub fn thread_routine(
    ...
    for u in start..end {
        ...
        info.store_estab(u, (deg, -(deg as i32)));
            ...
            while let Some(batch) = shared_pq.pop_batch(prio) {
                ...
                for (v, est_v) in activated.drain(..) {
                    if info.get_last_update(v) == iter {
                        ...
                        if local_queues.current_mut().vgc_enabled
                            && local_queues.current_mut().vgc_current
                                < local_queues.current_mut().vgc_threshold
                        {
                            local_queues.current_mut().get_vgc().push(v);
                            local_queues.current_mut().vgc_current += 1;
                            continue;
                        } else {
                ...
                for u in local_queues.current_mut().get_vgc().drain(..) {
                    info.set_last_update(u, iter);
    ...
    if threads_working.fetch_sub(1, Ordering::Relaxed) == 1 {
        println!(
            "[VGC: {}] Archi: {}, iterazioni: {}, sottoiterazioni totali: {}, attivazioni a vuoto: {}",
            vgc_t,
