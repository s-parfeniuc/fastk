...
pub mod estab {
    ...
    pub fn fetch_update_stability(
        ...
        match a.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |cur| {
            let est = self::estimate(cur);

            let s = self::stability(cur);

            if new_est < est && est as i32 + s.min(0) <= old_est as i32 {
                Some(self::pack(s.saturating_sub(delta), est))
            } else {
                None
            }
        }) {
...
impl LocalPQ {
    pub fn new(
        ...
        Self {
            ...
            mapper,
            vgc: Vec::with_capacity(vgc_threshold),
    ...
    pub fn get_vgc(&mut self) -> &mut Vec<usize> {
        &mut self.vgc
    }
}
...
impl Graph {
    ...
    pub fn fastk(
        ...
        let mapper = ExpoSubBuckets::new(self.max_deg, 512, 64);
...
pub fn thread_routine(
    ...
    let mut vgc: Vec<usize> = Vec::new();

    let mut subiter = 0;
    let mut iter: u16 = 0;
    let mut counter: usize = 0;
    let mut edges = 0;
    let mut vgc_current = 0;
    loop {
        iter += 1;
        ...
        loop {
            ...
            while let Some(batch) = shared_pq.pop_batch(prio) {
                edges += batch.edges;
            ...
            for (v, est_v) in activated.drain(..) {
                if info.get_last_update(v) == iter {
                    local_queues.next_mut().push_node(
                        est_v as usize,
                        v,
                        info.degree_of(v) as usize,
                    );
                } else {
                    if vgc_current < vgc_threshold {
                        info.set_last_update(v, iter);
                        vgc.push(v);
                        vgc_current += 1;
                        continue;
                    }
                    local_queues.current_mut().push_node(
                        est_v as usize,
                        v,
                        info.degree_of(v) as usize,
                    );
            ...
            for v in vgc.drain(..) {
                edges += info.degree_of(v) as usize;
                compute_index(v, &info, offsets, neighbors, &mut count, &mut activated, &mut temp, max_prio, &mut counter, tid);
            }
            for (v, est_v) in activated.drain(..) {
                if info.get_last_update(v) == iter {
                    local_queues.next_mut().push_node(
...
fn main() -> std::io::Result<()> {
    ...
    for line in reader.lines() {
        let line = line?;
        let value = line.parse::<u32>().unwrap_or(0);
        coreness.push(value);
    }

    println!("Per parsare il file: {:?}", start.elapsed());

    let vgc_values = [0, 32, 128, 256];
    ...
    for &vgc in &vgc_values {
        ...
        println!("[VGC {}]: {}, {}", vgc, total_diff, total_zeros);
    }
